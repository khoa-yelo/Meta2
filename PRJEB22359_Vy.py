...
df_final =  df_curated[df_curated['Study_ID'] == "PRJEB22359"]
df_original_subject = df_original[df_original["study_bioproject"] == "PRJEB22359"][["sample_accession", "sample_sample-name"]]
...
sample_accession_nucleotide_map = df_sra[df_sra['bioproject'] == "PRJEB9150"][["sample_accession", "nucleotide_type"]].set_index("sample_accession").to_dict()["nucleotide_type"]
sample_accession_age_map = df_sra[df_sra['bioproject'] == "PRJEB9150"][["sample_accession","age_years"]].set_index("sample_accession").to_dict()["age_years"] 
...
df_final["Nucleotide_Type"] = df_final["Sample_accession"].apply(lambda x: sample_accession_nucleotide_map[x])  #cột được thêm vào
df_final["Age"] = df_final["Sample_accession"].apply(lambda x: sample_accession_age_map[x])     #cột được thêm vào
df_final["Location"] = "Bangladesh"
df_final["Health_status"] ="diarrhea"
df_final["Age_Category"] = pd.cut(
    df_final["Age"],
    bins=[0, 2, 5, 17, 39, 64, np.inf],
    labels=["Infant", "Child", "Adolescent", "Young_adult", "Middle_age", "Elderly"],
...
df_final.to_csv("/content/drive/Shareddrives/Meta2/Metadata/Final_Curated_Out/PRJEB22359_Vy.csv", index = None)